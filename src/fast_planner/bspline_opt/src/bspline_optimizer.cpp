...
const int BsplineOptimizer::GRAVITY = (1 << 7);
...
void BsplineOptimizer::setParam(ros::NodeHandle& nh) {
  ...
  nh.param("optimization/roboheight", roboheight, -1.0);
...
void BsplineOptimizer::setCostFunction(const int& cost_code) {
  ...
  if (cost_function_ & GRAVITY) cost_str += " gravi |";
...
void BsplineOptimizer::calcGravityCost(const vector<Eigen::Vector3d>& q, double& cost,
                                        vector<Eigen::Vector3d>& gradient) {
  cost = 0.0;
  Eigen::Vector3d zero(0, 0, 0);
  std::fill(gradient.begin(), gradient.end(), zero);
...
void BsplineOptimizer::combineCost(const std::vector<double>& x, std::vector<double>& grad,
  ...
  double f_smoothness, f_distance, f_feasibility, f_endpoint, f_guide, f_waypoints, f_gravity;
  f_smoothness = f_distance = f_feasibility = f_endpoint = f_guide = f_waypoints = f_gravity = 0.0;
  ...
  if (cost_function_ & GRAVITY) {
    calcGravityCost(g_q_, f_gravity, g_gravity_);
    f_combine += lambda8_ * f_gravity;

    for (int i = 0; i < variable_num_ / dim_; i++)
      for (int j = 0; j < dim_; j++)
        grad[dim_ * i + j] += lambda8_ * g_gravity_[i + order_](j);
  }
