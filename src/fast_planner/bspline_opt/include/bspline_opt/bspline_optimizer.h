...
private:
  ...
  double lambda8_;                // gravity cost weight
                                  //
  double dist0_;                  // safe distance
  double dist1_;                  // zero gravity distance
  ...
  vector<Eigen::Vector3d> g_gravity_;
  ...
  void calcWaypointsCost(const vector<Eigen::Vector3d>& q, double& cost,
                         vector<Eigen::Vector3d>& gradient);
  void calcGravityCost(const vector<Eigen::Vector3d>& q, double& cost,
                                        vector<Eigen::Vector3d>& gradient); 
